...
class Token_Embedding(nn.Module):
    ...
    def __init__(self, token_size, d_model):
        ...
        self.iterval_embed = nn.Embedding(99,10, padding_idx=0)

    def forward(self, x, time, iterval):
        # Embed = self.token_embed(x) + self.time_embed(time)
        Embed = torch.cat((self.token_embed(x), self.time_embed(time), self.iterval_embed(iterval)), dim=-1)
...
class Bert_Embedding(nn.Module):
    ...
    def forward(self, x, time, iterval):
        ...
        x = self.dropout(self.token_embed(x, time, iterval) + self.posi_embed(x))
        return x
