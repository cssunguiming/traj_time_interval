...
class Predict_Model(nn.Module):
    ...
    def __init__(self, Bert_Traj_Model, token_size, head_n=12, d_model=768, N_layers=12, dropout=0.1):
        ...
        self.place_Linear = nn.Linear(d_model-20, token_size)
    ...
    def forward(self, x, input_time, max_len, iterval):
        x = self.bert(x, input_time, max_len, iterval)
        # logit: [batch_size, seq_size, d_]
        logit1 = self.place_Linear(x[:,:,:-20])
        logit2 = self.time_Linear(x[:,:,-20:-10])
